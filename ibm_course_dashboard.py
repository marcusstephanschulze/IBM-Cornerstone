...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        success_count = filtered_data[filtered_data['class'] == 1]['class'].count()
        failed_count = filtered_data[filtered_data['class'] == 0]['class'].count()

        fig = px.pie(values=[success_count, failed_count],
                 names=['Success', 'Failed'],
                 title=f"Success and Failure Rates for {entered_site} Launches" if entered_site != 'ALL' else "Total Success Launches by Site",
                 color_discrete_sequence=px.colors.sequential.Viridis)
